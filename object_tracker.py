...
flags.DEFINE_float('rate_sec', 0.1, 'time delay between two frames')
...
def main(_argv):
    ...
    while frame_num<num+2:
        ...
        # draw bbox on screen
            ...
            f = open("results.txt", "a+")
            f.write(f"{track.track_id} {(int(bbox[3]) + int(bbox[1]))/2} {(int(bbox[2]) + int(bbox[0]))/2} \n")
            f.close()
